...
function print_node(index::Int64, element::Int64, l::Float64)::Nothing
...
function print_element(k::Int64, n::Int16)::Nothing
    println("element $k")
    for local_node in 1:4
        ...
        print_node(global_node, k, n)
        println()
    end
end

"Print information about all elements."
function print_elements(n::Int16)::Nothing
    for element_idx in 1:4*n^2
        print_element(element_idx, n)
        println()
    end
end
...
function get_v_element(i::Int64, j::Int64, k::Int64, len::Float64, n::Int16)::Float64
    output = 0
    for l in 1:3
        for s in 1:3
    ...
    for k in 1:4*n^2
        for i in 1:4
            for j in 1:4
                ...
                H[global_index_1, global_index_2] += get_v_element(i, j, k, len, n) + get_t_element(i, j)
...
function handle_removing_things(H::Matrix{Float64}, S::Matrix{Float64}, i::Int64, n::Int16)::Nothing
    for j in 1:(2*n+1)^2
        H[i, j] = 0
        H[j, i] = 0
        S[i, j] = 0
...
function get_edge_indexes(n::Int16)::Int64[]
    output = []
    i = 1
    while i <= 2 * n + 1
        push!(output, i)
